...
class DepthCrafterPipeline(StableVideoDiffusionPipeline):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *args, **kwargs):
        # 1. Standard pipeline loading
        pipe = super().from_pretrained(pretrained_model_name_or_path, *args, **kwargs)

        # --- MANUAL VAE LOAD (stable version + VRAM optimization) ---
        # Reloading the VAE by hand and forcing decode_chunk_size=1 works around
        # cases where the VAE loaded by from_pretrained() decodes too many frames
        # at once and blows the VRAM budget on long clips.
        from diffusers import AutoencoderKLTemporalDecoder
        import os
        import torch
        from safetensors.torch import load_file

        vae_dir = os.path.join(str(pretrained_model_name_or_path), "vae")
        vae_file = os.path.join(vae_dir, "diffusion_pytorch_model.fp16.safetensors")
        pipe._local_vae_dir = vae_dir  # cached for the __call__ fallback below

        if os.path.exists(vae_file):
            try:
                print(f"[DepthCrafter] Manually loading VAE weights: {vae_file}")

                # 1. Initialize the module structure
                vae_config = AutoencoderKLTemporalDecoder.load_config(vae_dir)
                temp_vae = AutoencoderKLTemporalDecoder.from_config(vae_config).to(pipe.device, dtype=pipe.dtype)

                # 2. Load the weights
                state_dict = load_file(vae_file)
                temp_vae.load_state_dict(state_dict)

                # 3. Assign to the pipeline
                pipe.vae = temp_vae

                # 4. VRAM COMPENSATION: force decode_chunk_size to 1
                # This stops the VAE from trying to decode too many frames at once
                pipe.decode_chunk_size = 1

                print("[DepthCrafter] VAE loaded and decode_chunk_size set to 1.")
            except Exception as e:
                print(f"[DepthCrafter] Failed to manually load VAE weights: {str(e)}")
        else:
            print(f"[DepthCrafter] VAE file not found: {vae_file}")
    ...
    def __call__(
        ...
        # --- EMERGENCY VAE RE-INJECTION (last-resort fallback) ---
        if not hasattr(self, "vae") or self.vae is None:
            print("[DepthCrafter] VAE missing in __call__. Forcing an immediate reload...")
            from diffusers import AutoencoderKLTemporalDecoder
            from safetensors.torch import load_file
            import os

            vae_dir = getattr(self, "_local_vae_dir", None)
            if vae_dir:
                vae_file = os.path.join(vae_dir, "diffusion_pytorch_model.fp16.safetensors")
                try:
                    vae_config = AutoencoderKLTemporalDecoder.load_config(vae_dir)
                    self.vae = AutoencoderKLTemporalDecoder.from_config(vae_config).to(self.device, dtype=self.dtype)
                    self.vae.load_state_dict(load_file(vae_file))
                    print("[DepthCrafter] VAE re-injected successfully in __call__.")
                except Exception as e:
                    print(f"[DepthCrafter] Critical failure reloading VAE in __call__: {str(e)}")
            else:
                print("[DepthCrafter] No cached VAE directory available, cannot reload.")

        # Always force the chunk size to save VRAM
        ...
        while idx_start == 0 or idx_start < num_frames - overlap:
            ...
            # --- Overlap blending with a safety clamp (see note below) ---
            if latents_all is None:
                latents_all = latents.clone()
            else:
                if weights is not None:
                    # Clamp how many frames we actually overlap, to avoid a
                    # shape mismatch (e.g. 19 vs 2) on the last window
                    current_overlap = min(overlap, latents.shape[1], latents_all.shape[1])

                    # If the real overlap is smaller than requested, resize the weights
                    if current_overlap < overlap:
                        effective_weights = weights[:current_overlap]
                    else:
                        effective_weights = weights

                    # Blend safely
                    latents_all[:, -current_overlap:] = (
                        latents[:, :current_overlap] * effective_weights +
                        latents_all[:, -current_overlap:] * (1.0 - effective_weights)
                    )

                    # Append the rest of the new block
                    latents_all = torch.cat([latents_all, latents[:, current_overlap:]], dim=1)
                else:
        ...
        if latents_all is None:
            print(f"[DepthCrafter] Video too short ({num_frames} frames) for overlap ({overlap}).")
            print("[DepthCrafter] Forcing a single-segment computation...")

            # Prepare a single segment covering all frames
            latents = latents_init[:, :num_frames].clone()

            # Ideally this would run a simplified denoising loop, but to avoid
            # an immediate crash we just initialize latents_all directly:
            latents_all = latents
        ...
        if not output_type == "latent":
            # --- SAFETY FALLBACK ---
            # If self.vae is None, fall back to self.dtype to avoid an AttributeError.
            # In an fp16 setup the two usually match, so the result stays correct.
            target_dtype = self.dtype
            if hasattr(self, "vae") and self.vae is not None:
                target_dtype = self.vae.dtype

            print(f"[DepthCrafter] Cast latents to: {target_dtype} (VAE status: {self.vae is not None})")
            
            latents_all = latents_all.to(dtype=target_dtype)
